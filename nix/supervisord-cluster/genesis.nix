...
rec {
  ...
  files = ''
    ...
    TXOUT_AMOUNT=$(jq --arg addr "$TXIN_ADDR_HEX" \
                      --arg fee "$FEE" \
    '.initialFunds[$addr] - ($fee|tonumber) - (.protocolParams.poolDeposit + (2 * .protocolParams.keyDeposit) + ${toString genesis.delegator_coin}) * ${toString composition.n_pools}' < ${stateDir}/shelley/genesis.json)
    cardano-cli transaction build-raw \
        --ttl 1000 \
        --fee "$FEE" \
        --tx-in $(cardano-cli genesis initial-txin \
